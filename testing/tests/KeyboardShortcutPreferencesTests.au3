...
Local $themeIdList = GetThemeIdList()
...
; correct order.
Func RotatePersona_RotateAll($themeIdList)
   Local $sDescription
   Local $testPassed = False
   Local $result
...
Func RotatePersona_DifferentKeyAndChar($themeIdList)
   Local $sDescription
   Local $testPassed = False
   ...
   ; grabbing the id of the next theme
   Local $nextThemeId = $themeIdList[1]
...
Func RotatePersona_ExtraKey($themeIdList)
   Local $sDescription
   Local $testPassed = False
...
Func AutoSwitch_Disable()
   ...
   Else
	  $sDescription = "While the 'Switch every __ minutres' preference was enabled, the auto switch shortcut was able to disable it"
	  $testPassed = True
   EndIf
...
EndFunc
;------------------------------------------------------------------------------
; Helper function to grab the list of theme ids through rotation
Func GetThemeIdList()
   Local $cycle = True
   Local $themeList[0]
   While $cycle
	  Local $currentThemeId = _FFPrefGet("lightweightThemes.selectedThemeID")
	  _ArrayAdd($themeList, $currentThemeId)

	  Send("^!r")
	  Sleep(1000)

	  if _FFPrefGet("lightweightThemes.selectedThemeID") == $themeList[0] Then
		 $cycle = False
	  EndIf
   WEnd
